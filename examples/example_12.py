...
# 
# Tener en cuenta los comentarios incluidos.
# ---------------------------------------------------------------------------
    ...
    if results[0].boxes.id is not None:
        ...
        for box, track_id, class_id in zip(boxes, ids, class_ids):
            ...
            if track_id in track_hist:
                ...
                if prev_cy < line_y <= cy and track_id not in counted_out:
                    if 'car' in name:
                         crossing_colors[track_id] = (0, 0, 255)
                         counted_out.add(track_id)
                         car_out += 1
                    if 'truck' in name:
                         crossing_colors[track_id] = (255, 255, 102)
                         counted_out.add(track_id)
                         truck_out += 1
                    if 'bus' in name:
                         crossing_colors[track_id] = (102, 255, 204)
                         counted_out.add(track_id)
                         bus_out += 1                        
                        
                         
                if prev_cy > line_y >= cy and track_id not in counted_in:
                    if 'car' in name:
                         crossing_colors[track_id] = (255, 0, 255)
                         counted_in.add(track_id)
                         car_in += 1
                    if 'truck' in name:
                         crossing_colors[track_id] = (0, 153, 0)
                         counted_out.add(track_id)
                         truck_in += 1
                    if 'bus' in name:
                         crossing_colors[track_id] = (0, 153, 255)
                         counted_out.add(track_id)
                         bus_in += 1 

            if track_id in crossing_colors:
                cv2.rectangle(frame, (x1, y1), (x2, y2), crossing_colors[track_id], 1)
                         
                                     
            # stores the center (cx, cy) in the track_id position of track_hist
            track_hist[track_id] = (cx, cy)
        
    cv2.putText(frame, f'car_up: {car_in}', (60, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
    cv2.putText(frame, f'car_dw: {car_out}', (640, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
    cv2.putText(frame, f'bus_up: {bus_in}', (60, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
    cv2.putText(frame, f'bus_dw: {bus_out}', (640, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
    cv2.putText(frame, f'truck_up: {truck_in}', (60, 140), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
    cv2.putText(frame, f'truck_dw: {truck_out}', (640, 140), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
